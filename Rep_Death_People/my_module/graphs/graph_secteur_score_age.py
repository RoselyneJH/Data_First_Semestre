...
class   ClsGraphScoreAge:
    ...
    def render_graph_score_age_IMD(self,secteurs_originaires:bool=True,
                                        page:int=0, 
                                        indicateur :str="IMD") -> Tuple[go.Figure(), pd.DataFrame]:
        '''
        Initialise le traitement du graph
        ...
        if not vision_ville:
            ...
            df_heatmap = (
                df_score
                .pivot(index=self.nom_secteur, columns='classe_age', values=indicateur)
            )
            # réindexer les colonnes afin de respecter l'ordre
            df_heatmap = df_heatmap.reindex(columns=ordre_filtre)     
