...
def render_graph_bar_month(df_bar_month: pd.DataFrame, origine_secteur: str):
    ...
    fig.update_layout(
        title="Mortalité mensuelle et origine",  #
...
def render_graph_bar_class_age_month(df_bar_month_: pd.DataFrame, origine_secteur: str):
    ...
    df_bar_month = df_bar_month_.groupby(
        ["classe_age", "month_deces"], observed=True, as_index=False
    ).agg({"nb_deces": "sum"})

    # Je trie correctement
    df_bar_month["premier_numero"] = (
        df_bar_month["classe_age"].str.extract(r"(\d+)").astype(int)
    )
    ordre_cls_age_un_car = sorted(
        list(set(df_bar_month["premier_numero"])), reverse=False
    )

    df_bar_month["classe_age_1"] = pd.Categorical(
        df_bar_month["premier_numero"], ordre_cls_age_un_car, ordered=True
    )

    df_bar_month["month_deces_1"] = pd.Categorical(
        df_bar_month["month_deces"], ordre_mois, ordered=True
    )

    df_bar_month = df_bar_month.sort_values(["classe_age_1", "month_deces_1"]).drop(
        columns=["classe_age_1", "month_deces_1"]
    )
    ...
    for i, cls_age in enumerate(ordre_cls_age):
        if df_bar_month[df_bar_month["classe_age"] == cls_age]["nb_deces"].count() > 0:
            ...
            df_age = (
                df_age.assign(month_deces=df_age["month_deces"].astype(str).str.strip())
    ...
    fig.update_layout(
        title="Mortalité mensuelle et classes d'âge",  #
        plot_bgcolor="#ADD8E6",  # zone de tracé transparente (fond de la zone de tracé)
        paper_bgcolor="#ADD8E6",  # fond autour du tracé transparent (fond du “papier” autour du tracé)
        height=600,
        width=400,
