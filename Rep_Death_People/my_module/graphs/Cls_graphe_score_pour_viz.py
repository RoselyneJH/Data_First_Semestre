...
class ClsScorePourViz:
    ...
    def score_secteur(
        ...
        distance_non_origine = "distance_non_origine_" + self.cette_origine.replace(
            "origine_", ""
        )
        distance_origine = "distance_origine_" + self.cette_origine.replace(
            "origine_", ""
        )
        ...
        pl_cumul_secteur = (
            ...
            .with_columns(
                [
                    ...
                    .round(3)
                    .alias("TAFV"),  # Taux d'attractivité de fin de vie
                ]
            )
            .with_columns(
                [
                    ((pl.col("log_distance_secteur")) / pl.col("deces"))
                    .round(3)
                    .alias("mobilite_secteur"),  # mobilité du secteur mondérée.
                ]
            )
            .with_columns(
                [
                    ...
                    (
                        (pl.col("moy_distance") - pl.col("mobilite_secteur"))
                        / pl.col("std_distance")
                    )
                    .round(3)
                    .alias("IMD"),  # indice de mobilité diffe.
                ]
            )
            .collect()
        )
        ...
        df = pl_cumul_secteur.join(
            pl_cumul_secteur_restreint, on=la_liste_sans_filtre_age, how="inner"
        )
        ...
        df_cumul_secteur_.rename(
            columns={
                "item_nb_non_origine": nb_non_origine,
        ...
        for i in range(1, self.pages + 1):
            self.liste_df_cumul_secteur.append(
                df_cumul_secteur_[
                    (df_cumul_secteur_["rang_deces"] >= p)
                    & (df_cumul_secteur_["rang_deces"] < p + PAGE_SIZE)
                ]
            )
            p = (i * PAGE_SIZE) + 1
        ...
        self._nb_secteur_sans_deces_originaire = self.df_cumul_secteur[
            self.df_cumul_secteur[self.nb_origine] == 0
        ].shape[0]

        return df_cumul_secteur, "deces"  # nb_non_origine

    def identification_top_treemap(
        self, df: pd.DataFrame, la_ville: str, top_n: int = 5
    ):
        """
        ...
        top = (
            df_non_orig.group_by(niveau)
            .agg(pl.col("nb").max().alias("somme"))
    ...
    def preparation_treemap(self, df: pd.DataFrame, la_ville: str, top_n: int = 5):
        ...
        self.liste_departement_naissance_treemap = top[
            "nom_departement_naissance"
        ].to_list()
        ...
        df_treemap = pl.concat(
            [
                ...
                pl.DataFrame(
                    {"statut": ["Exogene"], "origine": ["Autres"], "valeur": [autres]}
                ).with_columns(pl.col("valeur")),
            ]
        )
