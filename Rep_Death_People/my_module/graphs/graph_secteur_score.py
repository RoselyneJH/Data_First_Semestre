    ...
    def render_graph_score(
        ...
        if len(self.df_score) >= 1 and not (self.origine_secteur == "origine_ville"):
            ...
            if self.class_filtrage.nb_secteur_sans_deces_originaire > 0:
                ce_nom_secteur = (
                    self.nom_secteur.replace("deces", "")
                    .replace("_", "")
                    .replace("nom", "")
                )
                texte_sur_secteur_sans_deces_originaire = "⚠️ Il y a " + str(
                    self.class_filtrage.nb_secteur_sans_deces_originaire
                )
                texte_sur_secteur_sans_deces_originaire = (
                    texte_sur_secteur_sans_deces_originaire
                    + " "
            ...
            if len(df_score_) > 0:
                fig.add_scatter(
                    ...
                    marker=dict(
                        ...
                        sizeref=2.0 * max(df_score_[self.nb_deces]) / (40.0**2),
                        opacity=0.6,
                    ),
                )
                ...
                fig.add_scatter(
                    ...
                    marker=dict(
                        ...
                        showscale=True,  # affiche la colorbar
                        sizemode="area",  # defini la zone d'utilisation homogenéité des marqueurs par defaut cette valeur
                        sizeref=2.0 * max(self.df_score[self.nb_deces]) / (40.0**2),
                        opacity=0.6,
                    ),
                )
