...
def render_graph_score(df_fnl: pd.DataFrame,
    ...
    # graphe :
    # Il faut rajouer une 2ieme condition (cas de la nouvelle caledonie qui ne 
    # possèdent aucun deces d'origianire) !!! non reglé & df_score[nb_origine].count()>0
    if len(df_score)>1 : # Alors secteur différent d'une ville 
        ...
        #    return fig_score, df_score
            ...
            tot_item_du_secteur = df_score.shape[0]
            tot_item_du_secteur_sans_originaire = df_score[df_score[nb_origine]==0].shape[0]
            df_score = df_score[df_score[nb_origine]>0].sort_values(nom_secteur, ascending = True)
            ...
            fig.add_scatter(
                ...
                marker=dict(
                    ...
                    colorbar=dict(# personnalise la colorbar
                        title="Décès exogènes",#"Locaux (nb)",
                        #tickvals=[0, 0.25, 0.5, 0.75, 1],
            ...
            yref="y",
            showarrow=False,
            text="<<< Attractivité des exogènes >>>",
            font=dict(size=14, color="blue"),
            )
            # Annotation pour justifier l'absence de secteurs sur l'axe des ordonnées
            if tot_item_du_secteur_sans_originaire > 0:
                ce_nom_secteur = nom_secteur.replace("deces","").replace("_","").replace("nom","")
                est_text_a_afficher = "Il y a "+str(tot_item_du_secteur_sans_originaire)
                est_text_a_afficher = est_text_a_afficher+" "+ce_nom_secteur +"(s)"
                est_text_a_afficher= est_text_a_afficher +" sans décès d'originaire."
                est_text_a_afficher= est_text_a_afficher +" Ces éléments ne figurent pas en ordonnées."
                fig.add_annotation(
                    x=0,                        # 
                    y=1.05,                        
                    xref="paper",
