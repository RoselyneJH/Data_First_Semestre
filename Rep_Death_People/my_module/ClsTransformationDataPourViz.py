...
class TransformationDataPourViz:
    ...
    def haversine_np(self, lat1: np.ndarray, lon1: np.ndarray, lat2: np.ndarray, 
    ...
    def ajout_distance_classe_age_origine(self, df_clean_ : pd.DataFrame) -> pd.DataFrame :
        '''
            Args    : Dataframe à transformer
        ...
        df_clean['distance'] = list(self.haversine_np( np.array(df_clean['latitude_naissance']),
        ...
        df_clean_nb_ville_origine = df_clean.query("origine_ville == 'O'").groupby(['annee','num_insee_deces'], as_index=False).agg(nb_originaire_ville = ('idligne','count'))
        df_clean_nb_departement_origine = df_clean.query("origine_departement == 'O'").groupby(['annee','num_insee_deces'], as_index=False).agg(nb_originaire_departement = ('idligne','count'))
        df_clean_nb_region_origine = df_clean.query("origine_region == 'O'").groupby(['annee','num_insee_deces'], as_index=False).agg(nb_originaire_region = ('idligne','count'))
        ...
        dfs = [df_clean_nb_deces_age_distance, df_clean_nb_ville_origine, df_clean_nb_departement_origine, df_clean_nb_region_origine, df_clean_nb_woman, 
        ...
        cols_a_modifier = ['nb_originaire_ville','nb_originaire_departement','nb_originaire_region',
                        'nb_woman','nb_deces_0_30','nb_deces_30_60','nb_deces_60_90','nb_deces_plus_90' ]
    ...
    def nettoyage_region_departement_latitude (self, df : pd.DataFrame ) -> pd.DataFrame :
        ...
        df_clean = df[df['latitude_naissance'].notna() & 
    ...
    def ExtractionDataTableDeathPeopleView(self) -> pd.DataFrame : 
        ...
        #df_clean_nan['classe_age'] = pd.cut(df_clean_nan['age'],
        #    bins=bins,labels=labels,duplicates='drop', right=False)
